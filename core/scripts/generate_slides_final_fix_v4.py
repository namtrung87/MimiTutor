"""
NotebookLM Automation V4 - The Persistence Fix
Goal: Create NEW notebook, Wait for UUID URL, Generate Slides, Return VALID URL
"""
import os
import sys
import time
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print("  Waiting for Notebook UUID URL...")
        final_url = ""
        for i in range(20): # Wait up to 20s for redirection
            url = page.url
            if "/notebook/" in url and "creating" not in url and "-" in url:
                # Basic check for UUID-like structure (IDs have dashes)
                final_url = url
                break
            await page.wait_for_timeout(1000)
            
        if not final_url:
            print(f"  Warning: URL did not stabilize to UUID. Current: {page.url}")
            final_url = page.url
             
        # CAPTURE URL
        print(f"  Captured URL: {final_url}")
        (OUTPUT_DIR / "final_notebook_url.txt").write_text(final_url, encoding='utf-8')
        
        # Inject Content
        print("  Injecting Content...")
        try:
            # Wait for chat input (ensures notebook is loaded)
...
"""
            ...
            (OUTPUT_DIR / "Unit8_NotebookLM_Slides_V4.txt").write_text(content, encoding='utf-8')
            await page.screenshot(path=str(OUTPUT_DIR / "Final_V4_Result.png"), full_page=True)
            print("  SUCCESS!")
